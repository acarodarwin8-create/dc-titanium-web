...
import { cursos, NIVEL_COLOR, NIVELES as NIVELES_CONTENT, ORDEN_CURSOS_PAGINA } from "@/content/cursos";

const NIVELES = ["Todos", ...NIVELES_CONTENT];

export default function CursosPage() {
  const [filtro, setFiltro] = useState("Todos");

  const cursosOrdenados = ORDEN_CURSOS_PAGINA.map((id) => cursos.find((c) => c.id === id)!)
    .filter((c) => c.activo)
            ...
            return (
              <div key={c.id} className="tb-glass-card" style={{ padding: "1.5rem", display: "flex", flexDirection: "column" }}>
                <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: "0.75rem" }}>
                  <span style={{ fontSize: "1.05rem", fontWeight: 800, color: c.color, fontFamily: "JetBrains Mono,monospace" }}>{c.software[0]}</span>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: "0.5rem", marginBottom: "0.9rem" }}>
                  <span className="tb-badge-norm">{c.certificacion}</span>
                ...
                <div style={{ marginTop: "auto" }}>
                  <div style={{ display: "flex", alignItems: "baseline", gap: "0.6rem", marginBottom: "1rem" }}>
                    ...
                    {desc > 0 && <span style={{ fontSize: "0.85rem", color: "#8B949E", textDecoration: "line-through" }}>${c.precioOriginal}</span>}
                  </div>
                  <Link href={"/cursos/" + c.slug} className="tb-btn-primary" style={{ textDecoration: "none", display: "block", textAlign: "center" }}>
                    Ver Hoja Técnica →
                  </Link>
                </div>
              </div>
